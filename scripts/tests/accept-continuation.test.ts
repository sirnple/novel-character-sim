...
export async function runAcceptContinuationTests(): Promise<void> {
  await suiteAsync("acceptContinuation", async () => {
    await testAsync("missing novelId → NO_NOVEL", async () => {
    ...
    await testAsync("missing / short draft → NO_DRAFT", async () => {
      _resetStore();
    ...
    await testAsync("missing non-main branch → NO_BRANCH", async () => {
      const userId = `tu_${randomUUID().slice(0, 8)}`;
      const novelId = `tn_${randomUUID().slice(0, 8)}`;
      try {
    ...
    await testAsync(
      "success: appends branch text, commits realized plants, clears store prose",
      async () => {
        _resetStore();
        const userId = `tu_${randomUUID().slice(0, 8)}`;
        const novelId = `tn_${randomUUID().slice(0, 8)}`;
        ...
        try {
          ...
          const beforeLen = (getBranch(userId, novelId, "main")?.text || "")
            .length;
          const r = await acceptContinuation({
            userId,
            novelId,
          ...
          assert.ok(
            (r.branchText || "").length > beforeLen,
            "branch text should grow",
          );
          assert.ok(
            (r.branchText || "").includes("顾深") ||
              (r.branchText || "").includes("旧桥"),
          );
          assert.ok(
            (r.activeCount ?? 0) >= 1,
            "ledger should plant from realized",
          );
          ...
          assert.ok(
            left === "" || left === undefined,
            `prose should be cleared, got: ${left}`,
          );
