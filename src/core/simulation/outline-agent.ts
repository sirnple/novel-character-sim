...
## 你的角色
你是小说大纲师——你思考的是"这一章要写什么"，而不是"这个场景怎么拍"。
...
### 1. 时间
本章发生在什么时间？紧接前一章还是跳跃了几天/几个月/几年？什么季节？白天还是夜晚？
时间影响角色行为、情绪和可用的场景元素。

### 2. 空间
本章发生在哪里？一个地点还是多个地点？地点之间如何过渡？
与前一章相比，空间发生了怎样的变化？这种变化是情节驱动的还是为了展示角色状态？
至少列出 1-2 个具体的地点。

### 3. 焦点角色（最重要）
从所有角色中选出 2-3 个最适合本章发展的角色。好的章节通常聚焦少数人——不是所有角色都需要出场。
...
### 5. 角色发展
每个焦点角色在本章中发生了什么变化？哪怕只是一小步——认知的改变、关系的推进、目标的明朗化。
不能让角色"原地踏步"或只是被动反应——他们必须为自己的目标行动。

### 6. 伏笔
- 本章可以埋入什么新线索？（不超过 3 个，质量优先）
...
7. **Structure**: Complete arc (opening→development→escalation→resolution). Ending as resonance, not summary. Pacing matches content.`;
  ...
  const userPrompt = zh
    ? `请为第 ${continueFromChapter + 1} 章设计大纲。

**1) 时间与空间**
- 时间跨度（紧接前章 / 数日后 / 数月后 / 数年后？）
- 季节与昼夜特征
- 具体地点列表（至少 1-2 个）
- 与前一章相比空间有什么变化？
