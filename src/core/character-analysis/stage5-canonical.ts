/**
 * Stage ④ canonicalName selection (legacy filename `stage5-canonical`).
 * Prefer importing from `./stage4-canonical`.
 *
 * Flow:
 ...
 *
 * Not a pipeline "agent" stage. After this, the pipeline ends; residual
 * same-person uncertainty is handled by the outer character-list agent tools.
 */
...
export function applyStage5CanonicalNames(
...
/** Stage ④ alias (preferred name for callers). */
export const applyStage4CanonicalNames = applyStage5CanonicalNames;
...
export async function applyStage5CanonicalNamesWithLlm(
  characters: MergedCharacter[],
  llm: LLMProvider | null | undefined,
  opts?: {
    concurrency?: number;
    scoreGapForLlm?: number;
    /**
     * Progress: (completed, total, canonicalName).
     * `completed` is finish-order count under concurrency (not character index).
     */
    onDone?: (completed: number, total: number, name: string) => void;
  },
  ...
  for (let i = 0; i < characters.length; i++) {
    ...
    if (pick.ruleConfident || !llm) {
      ...
      opts?.onDone?.(done, total, pick.canonicalName);
    } else {
      needLlm.push(i);
    }
  }
  ...
  // Pass 2: LLM only for uncertain (concurrent — report by completion order)
  let cursor = 0;
...
/** Stage ④ alias (preferred name for callers). */
export const applyStage4CanonicalNamesWithLlm = applyStage5CanonicalNamesWithLlm;
