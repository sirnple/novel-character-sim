...
- **`extractor/`** — Non-character extractors only: `StoryExtractor`, style/ideas/timeline, `novel-analysis-workspace`, `relationship-types`, `run-modular-extract`, `analysis-session`. Character extract/coref/workspace code lives under **`character-analysis/`** (including `runtime/`). Do not add character modules back under `extractor/`.
