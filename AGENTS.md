...
Provider selected via `LLM_PROVIDER` in `.env.local`. DeepSeek uses `OpenAIProvider` with `baseURL: https://api.deepseek.com/v1`. Each provider requires its own API key env var.
